...
int64_t GetProofOfWorkBonusRewardFactor(CBlockIndex* pindex)
{
    ...
    int64_t reward = GetProofOfWorkReward(pindex->nHeight, 0, pindex->pprev);
...
{
    int64_t nRewardCoinDay = MAX_MINT_PROOF_OF_STAKE_END;
    ...
    int64_t nSubsidy = nCoinAge * nRewardCoinDay;
...
bool CBlock::CheckBlock(bool fCheckPOW, bool fCheckMerkleRoot, bool fCheckSig) const
{
    ...
    if (GetBlockTime() > FutureDrift((int64_t)vtx[0].nTime))
        return DoS(50, error("CheckBlock() : coinbase timestamp is too early. GetBlockTime = %d > %d", 
...
static string strMintMessage = "INFO: Minting suspended.";
...
bool static ProcessMessage(CNode* pfrom, string strCommand, CDataStream& vRecv)
{
    ...
    if (strCommand == "version")
    {
        ...
        // ppcon: record my external IP reported by peer
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    //// debug print
    printf("CheckWork() : new block found  \n  hash: %s  \ntarget: %s\n", hash.GetHex().c_str(), hashTarget.GetHex().c_str());
    pblock->print();
...
void BitcoinMiner(CWallet *pwallet)
{
    ...
    printf("**** JackpotCoin Stake Minter has started ****\n");

    SetThreadPriority(THREAD_PRIORITY_LOWEST);

    RenameThread("JackPotCoin-miner");
    ...
    while (true)
    {
        ...
        else if (pblock->SignBlock(*pwallet, nFees))
        {
